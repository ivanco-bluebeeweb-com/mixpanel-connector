"""Connection lifecycle for Mixpanel Connector."""
...
@chat.function(
    "connect_mixpanel",
    "Connect Mixpanel account via credentials.",
    action_type="write",
    chain_callable=True,
    event="mixpanel-connector.connect_mixpanel",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_mixpanel(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Mixpanel Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Mixpanel Account",
...
@chat.function(
    "list_connections",
    "List connected Mixpanel accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_mixpanel",
    "Disconnect Mixpanel account.",
    action_type="write",
    chain_callable=True,
    event="mixpanel-connector.disconnect_mixpanel",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_mixpanel(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
